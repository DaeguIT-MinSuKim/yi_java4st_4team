...
FROM MENU_ORDER o JOIN MENU m ON o.CODE = m.CODE JOIN TABLEINFO t ON o.NO = t.NO WHERE o.NO = 2;
...
SELECT o.NO NO, o.CODE CODE, ORDERDAY, CNT, ISPAYMENT, m.NAME MNANE, PRICE, t.NAME TNAME FROM MENU_ORDER o JOIN MENU m ON o.CODE = m.CODE JOIN TABLEINFO t ON o.NO = t.NO;
SELECT o.NO NO, o.CODE CODE, ORDERDAY, CNT, ISPAYMENT, m.NAME MNANE, PRICE, t.NAME TNAME FROM MENU_ORDER o JOIN MENU m ON o.CODE = m.CODE JOIN TABLEINFO t ON o.NO = t.NO WHERE ISPAYMENT = 1 ORDER BY CNT DESC ;

SELECT ROW_NUMBER() OVER (ORDER BY SUM(CNT) * PRICE DESC) AS RANK, SUM(CNT) AS CNTALL, NAME, SUM(CNT) * PRICE AS SUMPRICE, ROUND(100 * (SUM(CNT) * PRICE)/(SELECT SUM(PRICE * CNT) AS SUMALL FROM MENU M LEFT OUTER JOIN MENU_ORDER O ON M.CODE = O.CODE), 2) AS PERCENTAGE FROM MENU_ORDER O LEFT OUTER JOIN MENU M ON O.CODE = M.CODE GROUP BY NAME, PRICE;

SELECT * FROM MENU_ORDER mo;
SELECT SUM(M.PRICE * CNT) AS SUMALL FROM MENU M LEFT OUTER JOIN MENU_ORDER O ON M.CODE = O.CODE;
